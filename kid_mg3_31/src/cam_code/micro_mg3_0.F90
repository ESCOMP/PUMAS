...
!++ag
logical, parameter, public :: ngcons = .false.
!--ag
!=========================================================
...
!++ag
real(r8), parameter :: ngnst = 0.1e6_r8     ! ice num concentration when nicons=.true. (m-3)
!--ag
...
!    pracs, npracs, (accretion of rain by snow)  psacr (collection of snow by rain)
       ...
       call graupel_rain_riming_snow(pracs(:,k),npracs(:,k),psacr(:,k),qsic(:,k), &
        ...
        call evaporate_sublimate_precip_graupel(t(:,k), rho(:,k), &
             ...
             qcic(:,k), qiic(:,k), qric(:,k), qsic(:,k), qgic(i,k), &
...
!++ag Watch sign of nscng and ngracs. What is sign of nnuccr? Negative? Shoudl be a source here. 
        ...
        if (dum.gt.ns(i,k)) then
           if (do_hail .or. do_graupel) then        
!Why is it prec_frac/(-nsubs....) ? Followed logic with nscng? Might be a problem with division by zero?
...
!             pracs(i,k)+mnuccri(i,k))*precip_frac(i,k)+berg(i,k))*xlf)
         ...
         tlat(i,k) = tlat(i,k)+((pre(i,k)*precip_frac(i,k)) &
             *xxlv+((prds(i,k)+prdg(i,k))*precip_frac(i,k)+vap_dep(i,k)+ice_sublim(i,k)+mnuccd(i,k)+mnudep(i,k)*lcldm(i,k))*xxls+ &
             ((bergs(i,k)+psacws(i,k)+mnuccc(i,k)+mnucct(i,k)+msacwi(i,k)+psacwg(i,k)+qmultg(i,k)+pgsacw(i,k))*lcldm(i,k)+(mnuccr(i,k)+ &
             pracs(i,k)+mnuccri(i,k)+pracg(i,k)+pgracs(i,k)+qmultrg(i,k))*precip_frac(i,k)+berg(i,k))*xlf)  
